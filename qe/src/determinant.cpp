...
int compute_phase_double_excitation(spin_det_t d, uint64_t h1, uint64_t h2,
                                    uint_64_t p1, uint_64_t p2) {
    // Single spin channel excitations, i.e., (2,0) or (0,2)
    int phase = compute_phase_single_excitation(d, h1, p1) *
                compute_phase_single_excitation(d, h2, p2);
...
spin_det_t apply_single_excitation(spin_det_t s, int spin, uint64_t h,
                                   uint64_t p) {
    assert(s[h] == 1);
    assert(s[p] == 0);
...
TEST_CASE("testing apply_single_excitation") {
    ...
    CHECK(apply_single_excitation(s, 0, 4, 1) ==
          det_t{spin_det_t{"01010"}, spin_det_t{"00001"}});
    CHECK(apply_single_excitation(s, 1, 0, 1) ==
        ...
        for (auto h2 = h1 + 1; h2 < h.size(); h2++) {
            for (auto p1 = 0; p1 < p.size() - 1; p1++) {
                for (auto p2 = p1 + 1; p2 < p.size(); p2++) {
                    res.push_back(apply_double_excitation(
                        d, std::pair<int>(spin, spin), h1, h2, p1, p2));
                }
            }
        }
    }
...
std::vector<det_t> get_constrained_singles(det_t d, exc_constraint_t constraint,
    ...
    spin_det_t hole_mask(
        to_string(constraint.first, max_orb)); // where holes can be created
    ...
    spin_constraint_t alpha_holes =
        to_constraint((~d[0] & hole_mask) & max_orb_mask);
    spin_constraint_t alpha_parts =
        to_constraint((d[0] & part_mask) & max_orb_mask);

    spin_constraint_t beta_holes =
        to_constraint((~d[1] & hole_mask) & max_orb_mask);
    spin_constraint_t beta_parts =
        to_constraint((d[1] & part_mask) & max_orb_mask);
    ...
    std::vector<det_t> alpha_singles =
        get_singles_by_exc_mask(d, 0, alpha_holes, alpha_parts);
    std::vector<det_t> beta_singles =
        get_singles_by_exc_mask(d, 1, beta_holes, beta_parts);
...
std::vector<det_t> get_all_singles(det_t d) {
    ...
    std::vector<det_t> singles;
    std::vector<det_t> alpha_singles = get_singles_by_exc_mask(
        d, 0, to_constraint(~d[0]), to_constraint(d[0]));
    std::vector<det_t> beta_singles = get_singles_by_exc_mask(
        d, 1, to_constraint(~d[1]), to_constraint(d[1]));
    singles.insert(singles.end(), alpha_singles.begin(), alpha_singles.end());
    singles.insert(singles.end(), beta_singles.begin(), beta_singles.end());
...
std::vector<det_t> get_constrained_ss_doubles(det_t d,
    ...
    std::vector<det_t> beta_ss_doubles =
        get_ss_doubles_by_exc_mask(d, 1, beta_holes, beta_parts);
    ss_doubles.insert(ss_doubles.end(), alpha_ss_doubles.begin(),
                      alpha_ss_doubles.end());
    ss_doubles.insert(ss_doubles.end(), beta_ss_doubles.begin(),
                      beta_ss_doubles.end());
