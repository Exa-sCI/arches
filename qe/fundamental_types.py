...
#         self.tag = archeslib.SPIN_DET_TYPE_BITSET
#         npa = np.array(sorted(t), dtype=np.intc)
#         npa_size = len(t)
#         self.handle = archeslib.arches_spin_det_bitset_create(npa, npa_size)
...
class Spin_determinant_tuple(Tuple[OrbitalIdx, ...]):
    ...
    def double_phase(
        ...
        -1
        """
        # TODO: NOTE, elsewhere in arches.drivers code, will have to switch hole particle order in args!
    ...
    def get_holes_particles_for_constrained_doubles(
        ...
        if nonconstrained_orbitals_occupied.popcnt() == 2:
            ...
            (
                x_a,
                y_a,
            ) = nonconstrained_orbitals_occupied  # This set will have length 2; unpack
            haa.append((x_a, y_a))
